...
with tf.Session() as __session_t:
  ...
  for i in range(__training_epochs):
    ...
    #region 输出shape
    print(__session_t.run(tf.shape(__h_conv1),feed_dict={__X_input:__x_batch}))
    print(__session_t.run(tf.shape(__h_pool1),feed_dict={__X_input:__x_batch}))
    print(__session_t.run(tf.shape(__h_conv2),feed_dict={__X_input:__x_batch}))
    print(__session_t.run(tf.shape(__h_pool2),feed_dict={__X_input:__x_batch}))
    #endregion
