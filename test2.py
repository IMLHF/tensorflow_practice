...
__sess = tf.InteractiveSession()
# __learning_rate=1e-4
# __training_epochs = 20000
# __batch_size = 50  # 每批训练数据的大小
# __display_step = 100  # 每隔__display_step批次显示一次进度
...
'''
tf.nn.max_pool 卷积函数
参数 value 输入图像               四维，shape如[batchnu_m, height, width, channels]
...
# 返回 Tensor
__logits = tf.matmul(__h_fc1_drop, __weights['__w_fc2']) + __biases['__b_fc2']
...
__train_op = tf.train.AdamOptimizer(1e-4).minimize(__loss_cross_entropy)
...
for i in range(20000):
  ...
  if i % 100 == 0:
    train_accuracy = __accuracy.eval(
...
print("test accuracy %g" % __accuracy.eval(
